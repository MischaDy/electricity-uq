...
class My_UQ_Comparer(UQ_Comparer):
    ...
    def compute_metrics(self, y_pred, y_quantiles, y_std, y_true, alphas=None):
        ...
        metrics = {  # todo: improve
            ...
            "mean_pinball": self._mean_pinball_loss(y_pred, y_quantiles, alphas) if y_quantiles is not None else None,
        }
        return metrics

    @staticmethod
    def _mean_pinball_loss(y_true, y_quantiles, alphas):
        """
        ...
        :return:
        """
        return np.mean([mean_pinball_loss(y_true, y_quantiles[:, ind], alpha=alpha)
    ...
    def posthoc_conformal_prediction(
        self, model, X_train, y_train, X_test, alphas=None, random_state=42
    ):
        cv_mapie_ts = BlockBootstrap(
            n_resamplings=10, n_blocks=10, overlapping=False, random_state=random_state
        )
        y_pred, y_quantiles = estimate_pred_interals_no_pfit_enbpi(
            model,
            cv_mapie_ts,
            alphas,
            X_test,
    ...
    def native_quantile_regression(self, X_train, y_train, X_test, alphas=None):
        y_pred, y_quantiles = estimate_quantiles_qr(X_train, y_train, X_test, alpha=alphas)
        # todo: reasonable with few quantiles?
        y_std = self.stds_from_quantiles(y_quantiles)
        return y_pred, y_quantiles, y_std
...
def main():
    ...
    uq_comparer.compare_methods(ALPHAS, should_plot_data=PLOT_DATA)
