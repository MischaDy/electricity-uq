...
from mapie_plot_ts_tutorial import train_base_model, estimate_pred_interals_no_pfit_enbpi
...
def main():
    ...
    model = train_base_model(RandomForestRegressor, model_params_choices=model_params_choices, X_train=X_train,
                             y_train=y_train, load_trained_model=True, cv_n_iter=10)
    ...
    labels_both = ['CP', 'QR']
    plot_intervals(X, y, X_train, y_train, X_test, y_test, y_preds_both, y_pis_both, labels_both)
...
def plot_intervals(X, y, X_train, y_train, X_test, y_test, y_preds_both, y_pis_both, labels_both):
    num_train_steps = X_train.shape[0]
    num_test_steps = X_test.shape[0]
    ...
    for i, (ax, y_pred, y_pis, label) in enumerate(zip(axs, y_preds_both, y_pis_both, labels_both)):
